...
/// macOS may prompt for Accessibility permission on first registration.
@MainActor
final class HotkeyManager {
    ...
    // No deinit: Carbon hotkey registrations and event handlers are
    // process-bound; the OS reclaims them on app exit. Adding cleanup here
    // would require crossing actor isolation in deinit, which Swift 6 forbids.

    func register(_ binding: Binding, action: @escaping () -> Void) {
        var hkRef: EventHotKeyRef?
        let hkID = EventHotKeyID(signature: OSType(0x53464C57), // "SFLW"
