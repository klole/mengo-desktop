...
final class FlowControllerTests: XCTestCase {
    ...
    func test_synthesize_successParentOutputDir_reviewsSlugDirectory() async {
        var t = Date(timeIntervalSince1970: 1_000_000)
        let parent = tmpDir().appendingPathComponent("skills", isDirectory: true)
        let skillDir = parent.appendingPathComponent("codex-smoke", isDirectory: true)
        ...
        try? #"{"schemaVersion":1,"steps":[]}"#.write(to: skillDir.appendingPathComponent("flow.json"), atomically: true, encoding: .utf8)

        let stub = RecordingSynthesis()
        stub.result = .success(outputDir: parent, slug: "codex-smoke")
        let c = makeController(synthesis: stub, settings: makeSettings(runtime: .codex), now: { t })

        await c.start()
        t = Date(timeIntervalSince1970: 1_000_030)
        await c.stop()

        guard case .reviewing(let reviewedDir) = c.flowState else {
            return XCTFail("expected .reviewing, got \(c.flowState)")
        }
        XCTAssertEqual(reviewedDir, skillDir)
        XCTAssertEqual(c.library.first?.path, skillDir)
    }
