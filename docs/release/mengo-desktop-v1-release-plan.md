...
- Added `scripts/smoke-app-launch.sh` for repeatable terminal launch smoke in local preview mode without starting capture.
- Added `scripts/release-readiness-status.sh` for repeatable terminal release-gate checks.
- Added `scripts/smoke-release-download.sh` for repeatable published-asset download, checksum, `ditto` extraction, codesign, optional extracted-app launch, and expected Gatekeeper-result verification.
...
- Added `scripts/manual-smoke-status.sh` to summarize the V1 manual smoke checklist, list exact open manual gates, and fail the release gate when `MENGO_REQUIRE_MANUAL_SMOKE_COMPLETE=1` is set for final V1.
...
- `MENGO_APP_LAUNCH_SMOKE=1 scripts/release-readiness-status.sh` can include terminal app-launch coverage: it starts `MengoDesktop.app` in signed-out, free local-preview, and pro local-preview modes with capture disabled, confirms the launch logs, and terminates the app cleanly.
