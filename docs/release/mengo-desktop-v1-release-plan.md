...
- Preview release downloads: 8, generated by repeated maintainer smoke checks.
...
- Manual smoke status is now machine-summarized: `scripts/manual-smoke-status.sh` reports the current checklist as 16/60 complete, with open GUI/manual gates listed by section. `scripts/manual-smoke-status.sh --require-complete` fails until the full clean-user checklist is complete.
