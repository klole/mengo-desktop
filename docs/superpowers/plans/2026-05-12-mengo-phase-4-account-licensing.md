...
- [ ] **Update memory** — `~/.claude/projects/-Users-your-user-screenpipe/memory/project_mengo_desktop_v2.md`: Phase 4 (account & licensing) shipped; key facts (magic-link sign-in / hard wall / offline-tolerant; `MENGO_DEV_ACCOUNT` dev hatch; Codex runtime wired; Stripe + mengo.ai endpoints are the website owner's job — contract documented in the Phase 4 spec).
